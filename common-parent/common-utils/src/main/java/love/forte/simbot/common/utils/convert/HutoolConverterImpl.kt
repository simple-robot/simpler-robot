 ...
 */
public open class HutoolConverterManagerImpl(private val converterRegistry: ConverterRegistry) : ConverterManager {
    /**
     * 获取某目标的转化器
     */
 ...
 */
private fun <T> Converter<T>.asHutoolConvert(): HutoolConverter<T> = HutoolConverterImpl { convert(it) }

/**
 * 将 [HutoolConverter] 接口实例 转化为 [Converter] 接口实例
 */
private fun <T> HutoolConverter<T>.asConverter(): Converter<T> = Converter<T> { convert(it, null) }
