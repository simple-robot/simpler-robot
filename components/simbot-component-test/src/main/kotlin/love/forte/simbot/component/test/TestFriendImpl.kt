 ...
 * @author ForteScarlet
 */
public class TestFriendImpl(
    override val bot: TestBot,
    override val id: ID,
    override val remark: String?,
    override val grouping: Grouping = Grouping.EMPTY,
    ...
    public companion object {
        public const val SEND_API_NAME: String = "friend.send"
    }
    ...
    override suspend fun send(message: MessageContent): MessageReceipt {
        configuration.delay()
        logger.debug("$bot -> $this :  $message")
        configuration.callback(bot, SEND_API_NAME, this, message)
        return TestMessageReceipt()
    }

    override suspend fun send(text: String): MessageReceipt {
        configuration.delay()
        logger.debug("$bot -> $this :  \"$text\"")
        configuration.callback(bot, SEND_API_NAME, this, text)
        return TestMessageReceipt()
    }
    ...
    override fun toString(): String = "Friend(id=$id, username=$username)"
