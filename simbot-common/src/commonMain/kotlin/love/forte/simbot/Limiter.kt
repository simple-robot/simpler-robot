 ...
 * 一个**限流器**。
 *
 * 通俗一点的理解，限流器可以理解为一个用于需要进行 **数量限制** 或者说需要 **分页** 的地方，常见于一些返回值为 [Flow], [Sequence] 或者 Java Stream 之类的地方。
 *
 * 对于限流的具体实现细节由对应功能的实现者自行决定（包括是否真的进行分页等）。
 *
 * [Flow] 有提供 [Flow.take] 、`java.util.Stream` 有提供 `Stream.limit` 之类的函数，但是 [Limiter] 更多的是为功能提供一个初始量。
 ...
 *
 * ## 扩展
 * Limiter提供了一些扩展函数来快速对 [flow][Flow]、[sequence][Sequence]、Java Stream 进行操作, 以flow为例：
 *
 * ```kotlin
 * val limiter: Limiter = ...
 ...
 * ```
 */
public operator fun Limiter.component1(): Int = offset
 ...
 * ```
 */
public operator fun Limiter.component2(): Int = limit
 ...
 * ```
 */
public operator fun Limiter.component3(): Int = batchSize
// endregion
