...
/**
 * 用于在 [Timestamp.byInstant] 中描述表示的时间类型。
 *
 * 在JVM平台中实现为 `java.time.Instant`,
 * 在其他平台中实现为 `kotlinx-datetime` 实现。
 *
 */
public expect class Instant

/**
 * 作为一些用于可以构建 [Instant] 的扩展函数的接收者。
 */
public object Instants

public expect val Instant.seconds: Long
public expect val Instant.secondNanos: Int
public expect fun Instants.fromEpochSeconds(second: Long, nanos: Int): Instant
public expect fun Instants.fromEpochMilliseconds(milliseconds: Long): Instant
public expect val Instants.DEFAULT_NOT_SUPPORT: Instant
public expect fun Instants.now(): Instant
...
@SerialName("ts")
public abstract class Timestamp : Comparable<Timestamp> {
    ...
    public companion object {
         ...
         */
        @JvmStatic
        public fun byInstant(instant: Instant): Timestamp =
            InstantTimestamp(instant.seconds, instant.secondNanos)
        
        /**
         * 根据当前时间瞬时构建一个 [Timestamp].
         */
        @JvmStatic
        public fun now(): Timestamp = nowTimestamp()
         ...
         * @param nanoAdjustment 针对 [epochSecond] 的纳秒偏移。有关nano偏移的概念类似于 [Instant.secondNanos]，有关更多内容参考 [Instant] 相关实现。
         */
        @JvmStatic
        @JvmOverloads
        ...
        public fun byMillisecond(epochMilli: Long): Timestamp {
            ...
            val secs = epochMilli.floorDiv(1000)
            val nos = (epochMilli.mod(1_000_000_000L) * 1_000_000L).toInt()
            
            return bySecond(secs, nos)
        }
    }
}

internal expect fun nowTimestamp(): Timestamp
...
public class InstantTimestamp internal constructor(
    ...
    override val millisecond: Long
        get() {
            ...
            return (second * 1000L) + (nanos / 1000000L)
        }
    ...
    public fun toInstant(): Instant {
        return Instants.fromEpochSeconds(second, nanos)
    }
...
/**
 * 将当前时间戳实例转化为 [Instant]。
 * 如果 [Timestamp] 满足 [isSupport][Timestamp.isSupport] != true，则会得到 [Instant.DISTANT_PAST]。
 */
public inline val Timestamp.instantValue: Instant
    get() = if (this is InstantTimestamp) toInstant() else if (isSupport()) Instants.fromEpochMilliseconds(millisecond) else Instants.DEFAULT_NOT_SUPPORT // Instant.EPOCH
