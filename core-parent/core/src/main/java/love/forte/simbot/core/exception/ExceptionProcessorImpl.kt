 ...
 * Project  parent
 * File     ExceptionProcessCenter.kt
 *
...
/**
 * [ExceptionProcessor] 的默认实现，通过解析实现类的泛型类型来决定对应的处理类型。
 *
 * 其中，如果类型为[RuntimeException]、[Exception]、[Throwable]，则会在对应类型无法处理的时候被使用。
 *
 */
public class ExceptionProcessorImpl(handles: Array<out ExceptionHandle<*>>) : ExceptionProcessor {
...
/**
 * [ExceptionProcessorBuilder] 的默认实现，可用于构建一个 [ExceptionProcessor] 实例。
 */
public class ExceptionProcessorBuilderImpl : ExceptionProcessorBuilder {
    ...
    /**
     * 构建 [ExceptionProcessorImpl] 实例并返回。
     */
    override fun build(): ExceptionProcessor = ExceptionProcessorImpl(handleList.toTypedArray())
