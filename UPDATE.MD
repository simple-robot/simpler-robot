...
### 核心
- [#40](https://github.com/ForteScarlet/simpler-robot/issues/40) 提供一个`AccountInfo`的子类型 `GroupAccountInfo`并为`群成员`等相关的信息提供一些额外的参数，例如用户头衔`accountTitle`，生效于与群成员有关的地方，例如群聊消息、获取到的群成员列表信息等。

- 弃用 `GroupAdmin`、`GroupOwner`、`GroupMemberInfo`、`BanInfo`、`FriendInfo`
  中的 `getAccountInfo`, 而是使得他们本身就属于 `AccountInfo`.

- 追加一个 `PrivateMsg(私聊事件)`的子事件类型 `FriendMsg(好友私聊事件)`, 其代表为一个确认为 **好友** 的私聊消息。

### Mirai
- 更新至 [v2.0-M2-2](https://github.com/mamoe/mirai/releases/tag/2.0-M2-2) 版本。
...
- [#32](https://github.com/ForteScarlet/simpler-robot/issues/32) 实现备用监听 `@SpareListen` 以及一个习惯用法 `@Spare`.
...
- 现在`BotInfo(bot信息)`也属于一种`AccountInfo(账号信息)`了。
...
### mirai
- 更新至 [v2.0.0](https://github.com/mamoe/mirai/releases/tag/2.0.0)
- 兼容 `record` 类型CAT码。
- 优化通过ID发送图片的时候的闪照处理。
...
- 暂停`component-ding`模块的更新，直到新的钉钉模块开发完成。在那之前，版本控制所管理的钉钉模块版本为 `2.0.0-RC.2`。
...
- [#52](https://github.com/ForteScarlet/simpler-robot/issues/52) 支持`kotlin`的`nullable`参数，使其等效于 `@Depend(orIgnore = true)`。
```kotlin
