...
/**
 * Tests for [DeleteBlacklistApi] focusing on API structure and request body serialization.
 * 
 * Reference: https://developer.kookapp.cn/doc/http/blacklist#移除黑名单
 * 
 * @author ForteScarlet
 */
class DeleteBlacklistApiTest {
    @Test
    fun testApiStructure() {
        val guildId = "test_guild_123"
        val targetId = "target_user_456"
        val api = DeleteBlacklistApi.create(guildId, targetId)
        
        // Test basic API properties
        assertEquals(HttpMethod.Post, api.method)
        assertTrue(api.url.toString().contains("blacklist/delete"))
        
        // Test URL path segments
        val url = api.url
        assertTrue(url.pathSegments.contains("blacklist"))
        assertTrue(url.pathSegments.contains("delete"))
    ...
    fun testApiResponseDeserializationStrategy() {
        val api = DeleteBlacklistApi.create("test_guild", "test_user")
    ...
    fun testApiRequestResult() = runTest {
        val guildId = "test_guild_delete"
        val targetId = "test_user_remove"
        val api = DeleteBlacklistApi.create(guildId, targetId)
        val authorization = "Bot test_token"
        
        // KOOK API success response for delete blacklist
        val apiResultJson = """{
            "code": 0,
            "message": "操作成功",
            "data": {}
    ...
    @Test
    fun testApiRequestWithDifferentIds() = runTest {
        val guildId = "different_guild_999"
        val targetId = "different_user_888"
        val api = DeleteBlacklistApi.create(guildId, targetId)
        val authorization = "Bot test_token"
