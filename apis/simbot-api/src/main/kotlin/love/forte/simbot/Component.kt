...
//  *
//  * [Component2] 的 [equals] 将会直接进行 `===` 匹配, 因此需要保证组件实例唯一性，开发者在 [Components] 中注册的时候，需要保证组件标识的ID唯一不变。
//  *
//  * 如果你想在对比两个 [Component2] 的时候，允许其中任意一方为 [SimbotComponent], 那么你需要使用 [Component2.like] 而不是 `equals`.
//  *
//  * @see Components
//  * @see Component2.like
...
// @Deprecated("Use Comp")
// public sealed class Component2 : Scope {
...
//  */
// public object SimbotComponent : Component2() {
...
//     override fun contains(scope: Scope): Boolean = scope is Component2
//
...
//  */
// public infix fun Component2.like(other: Component2): Boolean =
//     this === SimbotComponent || other === SimbotComponent || this === other


// /**
//  *
//  * 全局统一的 Component 管理器, 是 [Component2] 构建实例的唯一合法途径。
//  *
//  * 通过 [Components.create] 对 [Component2] 的实例进行统一管理，通过 [get]、[find] 或 [resolve] 来进行获取。
//  *
//  *
//  * @see Component2
//  * @see SimbotComponent
...
//     private val comps: ConcurrentHashMap<ID, Component2> = ConcurrentHashMap<ID, Component2>().also {
...
//
//     /**
//      * 创建一个对应 [id] 的 [Component2] 并记录。如果 [Component2] 已经存在，则抛出 [ComponentAlreadyExistsException].
//      *
...
//
//     /**
//      * 得到一个对应 [id] 的 [Component2] 实例，如果不存在则抛出 [NoSuchComponentException].
//      *
//      * @throws NoSuchComponentException 如果没有对应的 component
//      */
//     public operator fun get(id: ID): Component2 = find(id) ?: throw NoSuchComponentException(id.literal)
//     public operator fun get(id: String): Component2 = this[id.ID]
//
//
//     /**
//      * 寻找对应 [id] 的 [Component2] 实例，如果不存在则返回null。
//      *
//      */
//     public fun find(id: String): Component2? = find(id.ID)
//
//     /**
//      * 寻找对应 [id] 的 [Component2] 实例，如果不存在则返回null。
//      *
//      */
//     public fun find(id: ID): Component2? = comps[id.toCharSequenceID()]
//
//
//     @Suppress("MemberVisibilityCanBePrivate")
//     public val all: Sequence<Component2>
//         get() = comps.values.asSequence()
//
//     @Api4J
//     public fun all(): Stream<Component2> = all.asStream()
//
//
