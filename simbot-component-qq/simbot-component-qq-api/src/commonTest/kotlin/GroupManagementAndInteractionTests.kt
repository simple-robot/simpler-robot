...
class GroupManagementAndInteractionTests {
    ...
    fun interactionCreateDispatchResolveTest() {
        ...
        assertEquals(11, event.data.interactionType.value)
        assertEquals("group", event.data.interactionScene?.value)
        assertEquals(1, event.data.interactionChatType.value)
        assertEquals("group-openid", event.data.groupOpenid)
